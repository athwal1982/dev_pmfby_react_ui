...
const handleSubmit = async () => {
  ...
  questions.forEach((q) => {
    ...
    q.subQuestions?.forEach((subQ) => {
      if (
        feedbackResponses[q.id] === subQ.conditionId &&
        (!feedbackResponses[subQ.id] || !feedbackResponses[subQ.id].toString().trim())
      ) {
  ...
  const feedbackData = questions.map((q) => {
    ...
    if (q.options?.length > 0) {
      const selectedOption = q.options.find(
        (opt) => opt.id === feedbackResponses[q.id]
      );
      responseText = selectedOption
        ? selectedOption.option
        : feedbackResponses[q.id] || "";
    } else {
    ...
    q.subQuestions?.forEach((subQ) => {
      if (feedbackResponses[q.id] === subQ.conditionId) {
        const selectedSubOption = subQ.options?.find(
          (opt) => opt.id === feedbackResponses[subQ.id]
        );
        let subResponse = selectedSubOption
          ? selectedSubOption.option
          : feedbackResponses[subQ.id] || "";
    ...
    const mainOption = q.options?.find(
      (opt) => opt.id === feedbackResponses[q.id]
    );
    if (
      mainOption?.option?.toLowerCase().includes("other") &&
      feedbackResponses[`${q.id}_other`]
    ) {
...
const renderSubQuestions = (subQuestions, parentId) =>
  subQuestions.map((subQ) => {
    ...
    const hasAnyOtherOption = subQ.options?.some(
      (opt) => opt.option === "Any other"
    );
    const anyOtherOptionId = subQ.options?.find(
      (opt) => opt.option === "Any other"
    )?.id;
      ...
      >
        {subQ.options.length > 0 ? (
          <>
            ...
            {hasAnyOtherOption &&
              feedbackResponses[subQ.id] === anyOtherOptionId && (
                <TextField
                  fullWidth
                  variant="outlined"
                  value={feedbackResponses[`${subQ.id}_other`] || ""}
                  onChange={(e) =>
                    onFeedbackChange(`${subQ.id}_other`, e.target.value)
                  }
        ...
        {subQ.subQuestions?.length > 0 &&
          renderSubQuestions(subQ.subQuestions, subQ.id)}
      </Box>
    );
  });
...
return (
  <>
    {questions.map((q, index) => {
      const hasAnyOtherOption = q.options?.some(
        (opt) => opt.option === "Any other"
      );
      const anyOtherOptionId = q.options?.find(
        (opt) => opt.option === "Any other"
      )?.id;
      const hasSubQForAnyOther = q.subQuestions?.some(
        (subQ) => subQ.conditionId === anyOtherOptionId
      );
        ...
        >
          ...
          {q.options.length > 0 ? (
            <>
              ...
              {hasAnyOtherOption &&
                feedbackResponses[q.id] === anyOtherOptionId &&
                !hasSubQForAnyOther && (
                  <TextField
                    fullWidth
                    variant="outlined"
                    ...
                    onChange={(e) =>
                      onFeedbackChange(`${q.id}_other`, e.target.value)
                    }
                    placeholder="Please specify"
                    autoComplete="off"
                    sx={{ mt: 1 }}
              ...
              {q.subQuestions?.length > 0 &&
                renderSubQuestions(q.subQuestions, q.id)}
            </>
          ) : (
            <TextField
              fullWidth
