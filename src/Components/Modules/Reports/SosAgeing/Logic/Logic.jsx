...
import { getSupportTicketSosAgeingReport, getSupportSosAgeingReportDetail } from "../Services/Methods";

function SosAgeingReportLogics() {
  ...
  const [filteredSosAgeingReportDataList, setFilteredSosAgeingReportDataList] = useState([]);
  const [isLoadingSosAgeingReportDataList, setLoadingSosAgeingReportDataList] = useState(false);
  ...
  const downloadExcel = (data) => {
    ...
    XLSX.writeFile(workbook, "SosAgeing_Ticket_Details.xlsx");
  };
  ...
  const getSosAgeingReportsList = async (pViewMode) => {
    try {
      setLoadingSosAgeingReportDataList(true);

      const formData = {
        viewMode: pViewMode,
      };

      const result = await getSupportTicketSosAgeingReport(formData);
      setLoadingSosAgeingReportDataList(false);
      if (result.responseCode === 1) {
        setFilteredSosAgeingReportDataList(result.responseData.supportTicket);
        if (gridApi) {
          console.log(gridApi);
        }
      } else {
        setFilteredSosAgeingReportDataList([]);
      }
    } catch (error) {
      console.log(error);
      setAlertMessage({
  ...
  const onClickSosAgeingReport = () => {
    ...
    getSosAgeingReportsList(formValues.txtReportFilter.Value);
  };
  ...
  const exportSosAgeingTicketListClick = () => {
    ...
    const mappedData = SosAgeingTicketCountList.map((value) => {
      return {
  ...
  const [SosAgeingTicketCountList, setSosAgeingTicketCountList] = useState([]);
  const [isLoadingSosAgeingTicketCountList, setIsLoadingSosAgeingTicketCountList] = useState(false);
  const getSosAgeingReportsDetailsList = async (pViewMode, pFilterID, pageingPeriodsID, pticketStatusID) => {
    try {
      ...
      const result = await getSupportSosAgeingReportDetail(formData);
      setIsLoadingSosAgeingTicketCountList(false);
      if (result.responseCode === 1) {
        if (result.responseData.supportTicket && result.responseData.supportTicket.length > 0) {
          setSosAgeingTicketCountList(result.responseData.supportTicket);
        } else {
          setSosAgeingTicketCountList([]);
        }
      } else {
        setSosAgeingTicketCountList([]);
      }
...
export default SosAgeingReportLogics;
