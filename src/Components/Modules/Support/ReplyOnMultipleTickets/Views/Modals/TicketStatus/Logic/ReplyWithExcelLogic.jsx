...
import { getExcelBulkTicketsList, excelUploadData } from "../../../../Services/Methods";
      ...
      setTimeout(() => {
        ...
        const mappedData = pFarmerData.map((value) => {
          return {
            ...
            StateMasterName: value.PlotStateName,
            DistrictMasterName: value.PlotDistrictName,
          ...
          if (data.length <= 100) {
            try {
              ...
              if (result.responseCode === 1) {
                ...
                setsuccessStatusMsg("File is successfully uploaded. If the number of tickets are 100 or less than 100 it will processed now.");
                // A showfunc();
                // A setAlertMessage({
                // A  type: "success",
                // A  message: "Reply with excel file done successfuly,Please get the result excel file.",
                // A});
                const columnOrder = {
                  TicketNo: "Ticket No",
                  error: "Message",
            ...
            try {
              ...
              formDataDoc.append("insertedUserID", user && user.LoginID ? user.LoginID.toString() : "0");
              formDataDoc.append("insertedIPAddress", ip);
              formDataDoc.append("ImageName", pAttachmentName);
              formDataDoc.append("files", pAttachment);

              setBtnLoaderActive(true);
              const result = await excelUploadData(formDataDoc);
              setBtnLoaderActive(false);
              if (result.responseCode === 1) {
                setSuccessStatus(true);
                setsuccessStatusMsg("File is successfully uploaded. If the number of tickets are greater than 100 it will be processed during the day.");
                // A showfunc();
                // A setAlertMessage({
                // A type: "success",
                // A  message: "Reply with excel file done successfuly.",
                // A });
                handleResetFile();
                setFarmersTicketData([]);
                setSatatusCount(0);
