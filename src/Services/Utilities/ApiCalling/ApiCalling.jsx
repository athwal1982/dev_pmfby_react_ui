...
import { getTime } from "../../../Components/Common/Login/Services/Methods";

const pako = require("pako");

export const IfnullApiCalling = async (requestApiData, apiPath, header) => {
  try {
    const currdateTime = await getTime();
    if (!checkAuthExist(currdateTime.responseData.timestamp)) {
    ...
    // A    responseCode: 401,
    // A    responseData: null,
    // A    responseMessage: "Session Expired",
    // A  };
    // A }
...
export const apiCallingFormDatass = async (requestApiData, apiPath) => {
  try {
    const response = await axios({
      method: "post",
      url: Config.BaseUrl + apiPath,
...
export const ApiCallingReport = async (requestApiData, apiPath) => {
  debugger;
  try {
    const currdateTime = await getTime();
    if (!checkAuthExist(currdateTime.responseData.timestamp)) {
    ...
    const response = await axios.post(Config.ReportBaseUrl + apiPath, requestData, {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      },
    ...
    if (response && response.status === 200) {
      ...
      if (result.responseCode.toString() === "1") {
        ...
        if (buff.length !== 0) {
          const Data = JSON.parse(pako.inflate(buff, { to: "string" }));
          return { responseCode: 1, responseData: { responseData: Data, pagination: result.pagination }, responseMessage: result.responseMessage };
        }
        return { responseCode: 1, responseData: [], responseMessage: result.responseMessage };
      }
...
export const ApiCallingListing = async (requestApiData, apiPath) => {
  debugger;
  try {
    const currdateTime = await getTime();
    if (!checkAuthExist(currdateTime.responseData.timestamp)) {
...
export const apiCallingFormDataReportBaseUrl = async (requestApiData, apiPath) => {
  try {
    const response = await axios({
      method: "post",
      url: Config.ReportBaseUrl + apiPath,
      data: requestApiData,
      headers: {
        "Content-Type": "multipart/form-data",
        "Access-Control-Allow-Origin": "*",
